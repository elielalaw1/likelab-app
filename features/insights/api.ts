...
export type InsightsSummary = {
  ...
  // True when at least one (but not all) campaign's stats RPC failed, so the
  // shown totals understate reality and the UI can say so.
  partial: boolean
}

// Aggregates per-campaign performance from the campaign-stats RPC across every
// accepted campaign. This is real campaign-attributed performance (views/likes),
// distinct from the lifetime TikTok-account snapshot shown on the profile.
//
// NOTE: there is no time-series here — the backend stores only the current totals,
...
export async function getInsights(): Promise<InsightsSummary> {
  ...
  const results = await Promise.all(
    accepted.map(async (app): Promise<CampaignInsight> => {
      ...
      if (error) {
        ...
        failedCount++
        console.warn(`[insights] stats RPC failed for campaign ${app.campaignId}:`, error.message)
      }
  ...
  // If there were accepted campaigns but EVERY stats RPC failed, surface an
  ...
  // Keep only campaigns that actually have tracked engagement.
